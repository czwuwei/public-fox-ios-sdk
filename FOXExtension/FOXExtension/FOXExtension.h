//
//  FOXExtension.h
//  FOXExtension
//
//  Created by Wuwei on 2016/07/22.
//  Copyright © 2016年 CyberZ. All rights reserved.
//

#import <UIKit/UIKit.h>

//! Project version number for FOXExtension.
FOUNDATION_EXPORT double FOXExtensionVersionNumber;

//! Project version string for FOXExtension.
FOUNDATION_EXPORT const unsigned char FOXExtensionVersionString[];

// In this header, you should import all the public headers of your framework using statements like #import <FOXExtension/PublicHeader.h>


#import "FOXTutorialEvent.h"
#import "FOXRegistrationEvent.h"
#import "FOXAchieveLevelEvent.h"
#import "FOXAchieveStageEvent.h"
#import "FOXPurchaseEvent.h"